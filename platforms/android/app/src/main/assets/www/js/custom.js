...
$(window).load(function() { 
	...
	//set rootURL in form action
	if($("form[action*='rest']").attr("action") != undefined){
		var url = $("form[action*='rest']").attr("action");
		var urlFull = rootURL + url;
		$("form[action*='rest']").attr("action",urlFull);
	}
});

$( document ).ready(function() {
    
	//scroll page to set input field center of screen on click input text
	$('input').click(function(){
		var field = $(this);
		var intervalVar = setInterval(function(){ 
			$('.login-bg').scrollTop(field.offset().top + $('.login-bg').scrollTop());
			$('body #content').scrollTop(field.offset().top + $('body #content').scrollTop() - 200);
...
//This method is added in this file so that it can be accessed in all html files
function logout(){
	var r = confirm("Do you want to logout?");
    if (r == true) {
    	
    	// Initialize lokijs adapter allow 300mb memory
    	...
    	function databaseInitialize(){
    		var permissionCollection = db.getCollection("permissions");
    		
    		if (permissionCollection === null) {
    			console.log("permissions collection null");
    		}else{
    			console.log("permissions collection exist");
    			console.log(permissionCollection.data.length);
    			
    			// if collection exist clear all permission of user from collection
    			while(permissionCollection.data.length != 0){
    				for(i = 0; i < permissionCollection.data.length; i++){
    					permissionCollection.remove(permissionCollection.data[i]);
    				}
    			}
    			console.log(permissionCollection.data.length);
    		}
